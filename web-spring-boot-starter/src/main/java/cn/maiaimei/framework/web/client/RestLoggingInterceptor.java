...
public class RestLoggingInterceptor implements ClientHttpRequestInterceptor {
  ...
  public ClientHttpResponse intercept(HttpRequest request, byte[] body,
    ...
    if (log.isDebugEnabled()) {
      log.debug("URI: {}, Method: {}, Headers: {}, RequestBody: {}", request.getURI(),
          request.getMethod(), request.getHeaders(), requestBody);
    }
    final ClientHttpResponse response = execution.execute(getHttpRequestWrapper(request), body);
    final String responseBody = StreamUtils.copyToString(response.getBody(),
        Charset.defaultCharset());
    if (log.isDebugEnabled()) {
      log.debug("URI: {}, Status: {}, Headers: {}, ResponseBody: {}", request.getURI(),
