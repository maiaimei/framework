...
public class GlobalResponseHandler implements ApplicationContextAware, InitializingBean, ResponseBodyAdvice<Object> {
    ...
    public void afterPropertiesSet() {
        String basePackage = "";
        Map<String, Object> beansWithAnnotation = applicationContext.getBeansWithAnnotation(EnableGlobalResponse.class);
        ...
        for (String key : beansWithAnnotation.keySet()) {
            ...
            EnableGlobalResponse annotation = AnnotationUtils.findAnnotation(beansWithAnnotation.get(key).getClass(), EnableGlobalResponse.class);
            if (null != annotation) {
    ...
    public boolean supports(MethodParameter returnType, Class<? extends HttpMessageConverter<?>> converterType) {
        return Arrays.stream(basePackages).anyMatch(basePackage -> returnType.getDeclaringClass().getName().contains(basePackage));
    }
