package cn.maiaimei.framework.web.servlet.http;
...
/**
 * 请求响应日志过滤器 {@link ContentCachingRequestWrapper} {@link ContentCachingResponseWrapper}
...
public class HttpLoggingFilter extends HttpFilter {
  ...
  protected void doFilter(HttpServletRequest request, HttpServletResponse response,
    ...
    ReusableRequestWrapper reusableRequestWrapper = new ReusableRequestWrapper(request);
    ContentCachingResponseWrapper responseWrapper = new ContentCachingResponseWrapper(response);
    HttpInformation rrd = HttpUtil.getRequestResponseDetail(reusableRequestWrapper,
        excludeHeaderNames);
    long start = 0, end = 0;
    ...
    try {
      if (StringUtils.isNotBlank(source)) {
        MDCUtil.setSource(source);
      }
      MDCUtil.setTraceId(UUID.randomUUID().toString().replace("-", ""));

      start = System.currentTimeMillis();
      chain.doFilter(reusableRequestWrapper, responseWrapper);
  ...
  private boolean checkUri(HttpServletRequest request) {
    final String contextPath = StringUtils.defaultIfBlank(request.getContextPath(),
        StringUtils.EMPTY);
  ...
  private boolean isMultipartFormData(HttpServletRequest request, HttpServletResponse response) {
    return
        StringUtils.equalsIgnoreCase(request.getContentType(), MediaType.MULTIPART_FORM_DATA_VALUE)
            ||
            StringUtils.equalsIgnoreCase(response.getContentType(),
